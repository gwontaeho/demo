...
const GridContextProvider = ({ children, data }) => {
    ...
    if (Object.keys(_.current).length === 0) {
        ...
        _.current.rowStyles = [];
        for (let i = 0; i < data.length; i++) _.current.rowStyles[i] = { height: 40 };
    ...
    const handleScroll = throttle(({ target }) => {
        try {
            _.current.topIndex = _.current.rowStyles.findIndex(({ top, height }) => target.scrollTop < top + height);
            const { topIndex, overscanCount, rowStyles, headRef, totalDataCount } = _.current;
            ...
            _.current.endIndex = Math.min(
                topIndex +
                    overscanCount +
                    Math.ceil(
                        (target.getBoundingClientRect().height - headRef.getBoundingClientRect().height) /
...
const GridBody = () => {
    ...
    const { initalRender, startIndex, endIndex, rowStyles, totalDataCount, pagination, page, size } = _.current;
    ...
    return (
        ...
        >
            {view.map((item, index) => {
                const dataIndex = Math.min(page * size + startIndex + index, totalDataCount);
                const viewIndex = pagination ? startIndex + index : dataIndex;
                return <Row key={id + viewIndex} item={item} dataIndex={dataIndex} viewIndex={viewIndex} />;
            })}
        </div>
    );
};
...
const Row = (props) => {
    ...
    return (
        ...
        >
            <div className="w-10 border-r border-b items-center justify-center flex">{dataIndex + 1}</div>
            <div className="w-10 border-r border-b items-center justify-center flex">
                <input type="radio" />
            </div>
            <div className="w-10 border-r border-b items-center justify-center flex">
